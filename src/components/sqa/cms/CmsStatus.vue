<template>
  <div id="cmsStatus" style="width:100%; height:300px"></div>
</template>
...
export default {
  ...
  created() {
    ...
    for (let i = 0; i < _self.datas.length; i++) {
      let namedLabel = eval(localStorage.getItem("codeMissionStatus")).find(item => {
  ...
  mounted() {
    let _self = this;
    let charts = document.getElementById("cmsStatus");
