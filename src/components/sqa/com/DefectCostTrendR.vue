<template>
  <div id="defetctCostTrendR" style="width: 100%; height: 300px;"></div>
</template>
<script>
...
export default {
  ...
  created() {
    ...
    _self.datas.forEach(data => {
      ...
      trendData.push(data.defectCostAvg);
      defectCount.push(data.defectCount);
      defectCost.push(data.defectCost);
    });

    legendData.push('缺陷总数');
    seriesData.push({
      name: '缺陷总数',
      type: "bar",
      barMaxWidth: "50",
  ...
  mounted() {
    let _self =  this;
    let charts = document.getElementById("defetctCostTrendR");
