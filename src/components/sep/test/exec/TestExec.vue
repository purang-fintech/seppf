<template>
  <div id="root" style="width: 100%">
    ...
    <el-dialog :close-on-click-modal="modalClose" title="用例基本信息" :visible.sync="showDetail" width="1024px" top="100px" :fullscreen="maximize">
      <div slot="title">
      ...
      <case-info v-if="showDetail" :caseId="currentCase.id" :title="currentCase.name" :currentProd="productId" :readOnly="true"
    ...
    <el-dialog :close-on-click-modal="modalClose" title="新建缺陷" :visible.sync="showDefectSubmit" width="1130px" class="submit-defect"
      ...
      <el-form :model="defectForm" :rules="dFormRules" ref="ruledFormDefect" size="mini" :inline="true" label-width="110px">
        <el-form-item label="缺陷协调人" prop="conciliator" required>
          <el-select v-model="defectForm.conciliator" placeholder="请选择" clearable>
            <el-option-group
              v-for="group in testManagers"
              :key="group.label"
              :label="group.label">
              <el-option
    ...
    <el-dialog :close-on-click-modal="modalClose" title="测试用例历史运行记录" :visible.sync="showCaseRunHis" width="75%" :fullscreen="maximize2">
      ...
      <el-table :data="runHistory" size="mini" max-height="400" :border="showBorder" stripe empty-text="没有历史运行记录">
    ...
    <el-dialog :close-on-click-modal="modalClose" title="新的测试步骤运行" :visible.sync="showRunStep" class="run-steps" width="75%" :fullscreen="maximize3">
      <div slot="title">
    ...
    <div class="scenario" :style="{ height: baseHeight + 'px' }">
      <el-form size="small" class="scenario-header" inline>
        ...
        <el-form-item class="buttons">
          <el-button v-no-more-click type="primary" class="el-icon-circle-plus" size="small" plain @click="newScenarioCheck()"> 新建测试集</el-button>
        </el-form-item>
      </el-form>
      ...
      <el-table :data="scenarios"
        ...
        <el-table-column align="center" width="80">
          ...
          <template slot-scope="scope">
            <el-button v-no-more-click type="danger" size="mini" class="iconfont icon-empty icon-btn" circle
                       @click="delScenario(scope.row)"></el-button>
            <el-button v-no-more-click type="primary" size="mini" class="iconfont icon-addition icon-btn" circle @click="caseAddQuery()"></el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <div class="case-list" :style="{ height: baseHeight + 'px' }">
      <el-form size="small" class="case-header" inline>
        ...
        <el-form-item class="buttons">
          <el-button v-no-more-click type="primary" class="el-icon-remove" size="small" plain @click="batchDelScenarioCase()"> 批量移除</el-button>
        </el-form-item>
        <el-form-item class="buttons">
          <el-button v-no-more-click type="success" class="el-icon-success" size="small" plain @click="runScenarioCheck(1)"> 批量通过</el-button>
        </el-form-item>
        <el-form-item class="buttons">
          <el-button v-no-more-click type="danger" class="el-icon-error" size="small" plain @click="runScenarioCheck(0)"> 批量失败</el-button>
        </el-form-item>
      </el-form>
      <el-table :data="filterResult" :max-height="baseHeight - 36" size="small" stripe empty-text="该测试集未纳入测试用例" @selection-change="selectCases">
        <el-table-column type="selection" align="center" width="35">
        </el-table-column>
        <el-table-column label="调序" align="center" width="50">
          <template slot-scope="scope">
            <i v-show="scope.$index>0" class="iconfont icon-arrowup" style="cursor:pointer;font-size:12px;margin-right:2px"
               @click="indexUp(scope.$index)"/>
            <i v-show="scope.$index < cases.length - 1" class="iconfont icon-arrowdown" style="cursor:pointer;font-size:12px;"
               @click="indexDown(scope.$index)"/>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" width="110">
          ...
          <template slot-scope="scope">
            <el-button v-no-more-click type="danger" size="mini" class="iconfont icon-empty icon-btn" plain circle
                       @click="delScenarioCase(scope.row, scope.$index)"></el-button>
            <el-button v-no-more-click type="primary" size="mini" class="iconfont icon-history icon-btn" plain circle
                       @click="showCaseRunHistory(scope.row.id)"></el-button>
            <el-button v-no-more-click type="success" v-if="scope.row.type=='case'" size="mini" class="iconfont icon-run icon-btn" plain circle
  ...
  export default {
    data() {
      return {
        ...
        failedCaseId: "",
        dFormRules: {
          'conciliator': [{required: true, message: '请选择缺陷协调人', trigger: 'change'}],
          'req.selected': [{required: true, message: '请选择所属需求', trigger: 'change'}],
          'prodModule.selected': [{required: true, message: '请选择所属模块', trigger: 'change'}],
          'defectType.selected': [{required: true, message: '请选择缺陷类型', trigger: 'change'}],
          'foundPeriod.selected': [{required: true, message: '请选择发现阶段', trigger: 'change'}],
          'priority.selected': [{required: true, message: '请选择优先级', trigger: 'change'}],
          'influence.selected': [{required: true, message: '请选择严重程度', trigger: 'change'}],
          summary: [
            {required: true, message: '缺陷摘要', trigger: 'blur'},
            {min: 10, max: 50, message: '长度在 10 到 50 个字', trigger: 'blur'}
          ]
        },
        configs: {
      ...
      showDefectSubmit: function () {
        if (this.showDefectSubmit) {
          ...
          if (this.$refs["simpleMdeExecDefect"]) {
            [this.$refs["simpleMdeExecDefect"]].map(({simplemde}) => {
    ...
    methods: {
      ...
      relReqQuery(relId) {
        ...
        .then(function (res) {
          ...
          let currentRel = _self.release.opts.find(d => {return d.value == relId});
          relReqOpts.push({
