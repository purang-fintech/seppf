<template>
  <div id="root">
    ...
    <el-dialog title="缺陷拒绝说明" :close-on-click-modal="modalClose" :visible.sync="showBugRefuse" width="800px" :before-close="cancelRefuse">
      <el-form :model="refuseForm" size="small" :inline="false" label-width="120px" ref="refuseFormRef" :rules="refuseFormRules">
        ...
        <el-form-item label="拒绝说明" prop="refuseDetail" required>
          <el-input v-model="refuseForm.refuseDetail" type="textarea" style="width:95%" :rows="8" :maxlength="500" show-word-limit clearable></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer">
    ...
    <el-dialog title="选择修复负责人" :close-on-click-modal="modalClose" :visible.sync="showAssigin" width="600px">
      <el-select v-model="dfform.responser" placeholder="请选择" filterable :filter-method="filterUsers2" @visible-change="resetFilterText" style="margin-left:40px;width:85%">
        <el-option-group
    ...
    <el-dialog :close-on-click-modal="modalClose" title="缺陷处理说明" :visible.sync="showDescription" width="1024px" :fullscreen="maximize">
      ...
      <div slot="footer">
        ...
        <el-button v-no-more-click type="primary" v-if="quickOperation" icon="el-icon-circle-check" @click="updateDefectStatus()" size="small">保存</el-button>
        <el-button v-no-more-click type="primary" v-else icon="el-icon-circle-check" @click="updateDefect()" size="small">保存</el-button>
      </div>
    </el-dialog>

    <el-dialog :close-on-click-modal="modalClose" title="提交新缺陷" :visible.sync="showDialog" width="1130px" class="new-defect" :fullscreen="maximize1">
      <div slot="title">
      ...
      <el-form :model="dfform" :rules="dFormRules" ref="ruledForm" size="mini" :inline="true" label-width="110px">
        ...
        <el-form-item label="修复责任人" prop="responser">
          <el-select v-model="dfform.responser" placeholder="请选择" :disabled="!roleAllow(isManager)" filterable :filter-method="filterUsers1"
    ...
    <el-dialog
      ...
      <el-form :model="dfform" size="mini" :rules="dFormRules" ref="ruledFormMod" :inline="true" label-width="110px">
        <el-form-item label="提交人" required>
        ...
        <el-form-item label="修复责任人" prop="responser">
          <el-select v-model="dfform.responser" placeholder="请选择" :disabled="currentRow.disabled || !roleAllow(isManager)" filterable
                     :filter-method="filterUsers2" @visible-change="resetFilterText">
            <el-option-group
              v-for="group in userOptions2"
              :key="group.label"
              :label="group.label">
      ...
      <div slot="footer" style="padding:0 35px 10px 0">
        ...
        <el-button v-no-more-click type="primary" icon="el-icon-circle-check" @click="saveUpdateDefect('ruledFormMod')" size="small"
    ...
    <div class="dform-box">
      <el-form ref="dform" :model="dform" :inline="true" size="mini" label-width="100px" @keydown.native.enter="queryDefect()">
        ...
        <el-form-item label="提交人">
          <el-select v-model="dform.submitter" placeholder="请选择" clearable filterable :filter-method="filterUsers3" @visible-change="resetFilterText">
            <el-option-group
        ...
        <el-form-item label="修复责任人">
          <el-select v-model="dform.responser" placeholder="请选择" clearable filterable :filter-method="filterUsers" @visible-change="resetFilterText">
            <el-option-group
      ...
      <el-table :data="dataRevert" :max-height="baseHeight" size="mini" stripe :border="showBorder" ref="defectTable"
        ...
        <el-table-column prop="relCode" label="发现版本" width="120" align="center" sortable show-overflow-tooltip>
        </el-table-column>
  ...
  export default {
    data: function () {
      return {
        ...
        refuseFormRules: {
          ...
          'sameCodeDefect': [{required: true, message: '请选择目标合并缺陷', trigger: 'change'}],
          'refuseDetail': [
            {required: true, message: '请输入拒绝理由描述', trigger: 'blur'},
            {min: 5, max: 500, message: '长度在 5 到 500 个字', trigger: 'blur'}
          ]
        },
        refuseReasons: [],
        dFormRules: {
          'relId': [{required: true, message: '请选择所属版本', trigger: 'change'}],
          'reqId': [{required: true, message: '请选择所属需求', trigger: 'change'}],
          ...
          'foundPeriod': [{required: true, message: '请选择发现阶段', trigger: 'change'}],
          'status': [{required: true, message: '请选择缺陷状态', trigger: 'change'}],
          summary: [
            {required: true, message: '请输入缺陷摘要', trigger: 'blur'},
      ...
      showDialog: function () {
        if (this.showDialog) {
          ...
          if (this.$refs["simpleMdeNew"]) {
            [this.$refs["simpleMdeNew"]].map(({simplemde}) => {
    ...
    methods: {
      ...
      showDetail(data) {
        ...
        this.$nextTick(() => {
          [this.$refs.simpleMdeMod].map(({simplemde}) => {
      ...
      relReqQuery(relId) {
        ...
        .then(function (res) {
          ...
          let currentRel = _self.releases.find(d => {return d.value == relId});
