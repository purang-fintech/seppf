<template>
  <div id="root" class="require-main">
    ...
    <el-dialog :close-on-click-modal="modalClose" title="产品需求附件清单" :visible.sync="showAttachDialog" width="960px" top="100px">
      <el-table :data="currentRequest.attachments" size="mini" class="detail-table" empty-text="当前需求没有上传附件" stripe max-height="500px">
    ...
    <el-dialog :close-on-click-modal="modalClose" title="需求审批情况" :visible.sync="showAuditHistory" width="910px" top="100px">
      <div class="audit-history" v-if="auditHistories.length>0">
        <el-timeline reverse>
          <el-timeline-item
            v-for="(item, index) in auditHistories"
    ...
    <el-dialog title="需求送审" :close-on-click-modal="modalClose" :visible.sync="showSendAudit" width="600px">
      <el-form :model="auditForm" size="small" :inline="false" label-width="120px" ref="sendAuditForm">
        ...
        <el-form-item label="主管审批人" required prop="leaderAuditor" :rules="[{ required: true, message: '主管审批人必选', trigger: 'change' }]">
          ...
          <el-select v-if="currentRequest.type==2" v-model="auditForm.leaderAuditor" placeholder="请选择" multiple clearable style="width:92%">
            <el-option v-for="item in auditForm.devManagers" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
          <el-select v-else v-model="auditForm.leaderAuditor" placeholder="请选择" multiple clearable style="width:92%">
        ...
        <el-form-item label="高管审批人">
          <el-select  v-if="currentRequest.type==2" v-model="auditForm.chiefAuditor" placeholder="请选择" multiple clearable style="width:92%">
            <el-option v-for="item in auditForm.itChiefs" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
          <el-select  v-else v-model="auditForm.chiefAuditor" placeholder="请选择" multiple clearable style="width:92%">
    ...
    <el-dialog :close-on-click-modal="modalClose" :visible.sync="showDialog" width="960px" :fullscreen="maximize1" top="100px">
      ...
      <div v-loading.lock="checking"
        ...
        <el-form :model="qform" :rules="qFormRules" ref="ruledForm" size="mini" :inline="true" label-width="110px" class="request-form">
    ...
    <el-dialog :close-on-click-modal="modalClose" :visible.sync="showDialogMod" width="960px" :fullscreen="maximize2" top="100px" :before-close="cancelView">
      <div slot="title">
      ...
      <div v-loading.lock="checking"
           ...
           element-loading-spinner="el-icon-loading"
           element-loading-background="rgba(0, 0, 0, 0.8)">
        <el-form :model="qform" size="mini" :rules="qFormRules" ref="ruledFormMod" :inline="true" label-width="110px" class="request-form">
          <el-form-item label="需求类型" prop="type" required>
          ...
          <el-form-item label="期望完成日期" prop="expectDate" required>
            <el-date-picker type="date" v-model="qform.expectDate" :value-format="datefmt" placeholder="请选择" clearable :disabled="qform.disabled"></el-date-picker>
          </el-form-item>
          <el-form-item label="需求摘要" prop="summary" required>
          ...
          <el-form-item label="需求详情" prop="detail">
            <el-input v-model="qform.detail" type="textarea" :rows="3" placeholder="请输入产品需求详情" class="req-attach-upload" :maxlength="2000"
                      :disabled="qform.disabled" show-word-limit clearable>需求详情
            </el-input>
          </el-form-item>
          <el-form-item label="附件上传">
            <el-upload
      ...
      <div slot="footer">
        ...
        <el-button v-no-more-click v-if="!qform.disabled" type="primary" icon="el-icon-circle-check" @click="checkReqUpdate('ruledFormMod')" size="small">确定</el-button>
      </div>
    </el-dialog>

    <div class="request-main">
      <el-form ref="form" :model="form" :inline="true" size="mini" label-width="110px" @keydown.native.enter="requestQuery()">
        <el-form-item label="需求类型">
          <el-select v-model="form.type" placeholder="请选择" clearable multiple collapse-tags class="muti-select">
            <el-option v-for="opt in reqType" :value="opt.value" :key="opt.value" :label="opt.label" :disabled="opt.disabled"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="提交人">
          <el-select v-model="form.submitter" placeholder="请选择" filterable clearable :filter-method="filterUsers" @visible-change="resetFilterText">
            <el-option-group
      ...
      <div class="page-set">
        <el-pagination
          ...
          :page-size="pageSize"
          layout="total, sizes, prev, pager, next, jumper"pm run dev
  ...
  export default {
    data: function () {
      return {
        ...
        qFormRules: {
          ...
          detail: [
            {required: true, message: '请输入产品需求详情', trigger: 'blur'},
    ...
    methods: {
      ...
      sendAuditRequest(data) {
        let _self =  this;
        const crtModule = _self.modules.find(d => {return d.value == data.moduleId});
        _self.$nextTick(() => {
          _self.auditForm.pdAuditor = {value: crtModule.pduid, label: crtModule.pdname};
      ...
      queryDevManagers(){
        ...
        commonQuery.roleMemberQuery(sessionStorage.productId, 3, (result) => {
          _self.auditForm.devManagers = result.users.filter(d => {return [8249, 8445].indexOf(d.value) == -1});
        });
      },
