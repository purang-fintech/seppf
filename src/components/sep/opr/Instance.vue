<template>
  <div class="instance-root" style="width: 100%">
          ...
          <template slot-scope="scope">
            ...
            <el-tooltip effect="dark" content="环境配置" placement="bottom">
              <el-button icon="el-icon-tickets"
                         @click="$router.push({ name: 'instanceEnv', query: { 'instance': scope.row.instance ,'instanceType':scope.row.type} })"
                         type="text"
                         class="oper-button" style="color:#3AB4D7"></el-button>
            </el-tooltip>
            <el-button icon="el-icon-delete" @click="deleteInstanceConfirm(scope.row)" type="text" class="oper-button"
    ...
    <!--添加或修改-->
    <el-dialog width="40%" :title="instanceFormTitle" :visible.sync="dialogInstanceVisible"
               :close-on-click-modal="modalClose" @close="clearForm('instanceForm')">
      <el-form label-width="100px" :model="instanceForm" ref="instanceForm" :rules="rules" class="demo-ruleForm">
        ...
        <el-form-item label="实例类型" prop="type" required>
          <el-select v-model="instanceForm.type" placeholder="实例类型" style="width:95%">
            <el-option v-for="type in instanceTypes" :label="type.value" :value="type.name"
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        rules: {
          instance: [
            {validator: checkInstance, trigger: 'blur'}
          ],
          type: [
