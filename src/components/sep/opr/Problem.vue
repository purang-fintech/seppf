<template>
  <div id="root" style="width: 100%">
    ...
    <el-dialog title="问题拒绝说明" :close-on-click-modal="modalClose" :visible.sync="showProRefuse" width="800px" :before-close="cancelRefuse">
      <el-form :model="refuseForm" size="small" :inline="false" label-width="120px" ref="refuseFormRef">
        <el-form-item label="拒绝说明" prop="refuseReason" required :rules="[{required: true, message: '请输入拒绝理由描述', trigger: 'blur'},
          ...
          <el-input v-model="refuseForm.refuseReason" type="textarea" style="width:95%" :rows="8" :maxlength="500" show-word-limit clearable></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer">
    ...
    <el-dialog :close-on-click-modal="modalClose" :visible.sync="showImprove" width="750px" :before-close="cancelImprove">
      <el-form :model="currentProblem" size="mini" :inline="false" label-width="140px">
        ...
        <el-form-item label="改进计划完成日期" prop="improvePlanTo" required>
          <el-date-picker type="date" v-model="currentProblem.improvePlanTo" :value-format="datefmt" placeholder="请选择" clearable
                          style="width:250px"></el-date-picker>
        </el-form-item>
        <el-form-item label="改进措施详情" prop="improveDetail" required
                      :rules="[{ min: 0, max: 2000, message: '长度不可超过 2000 字，内容过多请上传附件', trigger: 'blur' }]">
          <el-input v-model="currentProblem.improveDetail" type="textarea" :rows="8" placeholder="请输入改进措施详情" :maxlength="2000" show-word-limit
                    clearable>改进措施详情
          </el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog :close-on-click-modal="modalClose" title="提交线上问题" :visible.sync="showDialog" width="960px" :fullscreen="maximize" top="100px">
      <div slot="title">
      ...
      <el-form :model="pbform" size="mini" :rules="proFormRules" ref="ruledFormNew" :inline="true" label-width="115px" class="new-pform">
        ...
        <el-form-item label="负责人" prop="responser" required>
          <el-select v-model="pbform.responser" placeholder="请选择" clearable filterable :filter-method="filterUsers2"
        ...
        <el-form-item label="须解决时间" prop="expectResolveTime" required>
          <el-date-picker
            ...
            v-model="pbform.expectResolveTime"
            placeholder="请选择须解决时间">
          </el-date-picker>
        </el-form-item>
        ...
        <el-form-item label="问题详情" prop="detail">
          <el-input v-model="pbform.detail" type="textarea" :rows="2" placeholder="请输入问题详情" class="pro-attach-upload" :maxlength="2000"
                    show-word-limit clearable>问题详情
          </el-input>
        </el-form-item>
        <br>
    ...
    <el-dialog :close-on-click-modal="modalClose" title="线上问题处理" :visible.sync="showDialogMod" width="960px" :fullscreen="maximize1" top="100px" :before-close="cancelView">
      <div slot="title">
      ...
      <el-form :model="currentProblem" size="mini" :rules="currentProblemRules" ref="ruledFormEdit" :inline="true" label-width="115px" class="new-pform">
        ...
        <el-form-item label="须解决时间" prop="expectResolveTime" required>
          <el-date-picker :disabled="currentProblem.disabled"
            type="datetime"
            :value-format="timefmt"
            v-model="currentProblem.expectResolveTime"
            placeholder="请选择须解决时间">
          </el-date-picker>
        </el-form-item>
        ...
        <el-form-item label="负责人" prop="responser" required>
          <el-select v-model="currentProblem.responser" placeholder="请选择" clearable filterable :filter-method="filterUsers3" :disabled="currentProblem.disabled"
                     @visible-change="resetFilterText">
            <el-option-group
        ...
        <br>
        <el-form-item label="问题详情" prop="detail">
          <el-input v-model="currentProblem.detail" type="textarea" :rows="2" placeholder="请输入问题详情" class="pro-attach-upload" :maxlength="2000"
                    :disabled="currentProblem.disabled" show-word-limit clearable>问题详情
          </el-input>
        </el-form-item>
        <br>
        <el-form-item label="附件上传">
      ...
      <div slot="footer">
        ...
        <el-button v-no-more-click type="primary" icon="el-icon-delete" :disabled="refuseDenined()" @click="refuseProblem()" size="small">拒绝</el-button>
        <el-button v-if="!currentProblem.disabled" v-no-more-click type="primary" icon="el-icon-edit" @click="showImprove=true" size="small">改进措施</el-button>
        <el-button v-if="!currentProblem.disabled" v-no-more-click type="primary" icon="el-icon-circle-check" @click="checkProblemEdit('ruledFormEdit')" size="small">确定</el-button>
      </div>
    </el-dialog>
    ...
    <div class="pro-form">
      <el-form ref="form" :inline="true" size="mini" label-width="100px" @keydown.native.enter="problemQuery()">
        ...
        <el-form-item label="上报日期">
          <el-date-picker v-model="form.submitTime" type="daterange" align="right" unlink-panels :value-format="datefmt" range-separator="至"
                          start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickOptions">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="解决日期">
          <el-date-picker v-model="form.resolveTime" type="daterange" align="right" unlink-panels :value-format="datefmt" range-separator="至"
                          start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickOptions">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="关闭日期">
          <el-date-picker v-model="form.closeTime" type="daterange" align="right" unlink-panels :value-format="datefmt" range-separator="至"
                          start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickOptions">
          </el-date-picker>
        </el-form-item>
            ...
            <el-form label-position="right" inline class="pro-table-expand" size="mini">
              ...
              <el-form-item label="改进措施详情">
                <div class="content-tips">
                  <el-popover
                    placement="right-end"
                    :disabled="!scope.row.improveDetail"
                    width="600"
                    trigger="click">
...
<script>
  ...
  import {dateFormat, dateDiff, pickOptions} from "@/util/date.js";
    ...
    data: function () {
      return {
        ...
        proFormRules: {
          ...
          influence: [{required: true, message: '请选择严重程度', trigger: 'change'}],
          responser: [{required: true, message: '请选择负责人', trigger: 'change'}],
          typeFirst: [{required: true, message: '请选择一级分类', trigger: 'change'}],
          'typeSecond.selected': [{required: true, message: '请选择二级分类', trigger: 'change'}],
          resolveMethod: [{required: true, message: '请选择问题处理方式', trigger: 'change'}],
          moduleId: [{required: true, message: '请选择产品模块', trigger: 'change'}],
          expectResolveTime: [{required: true, message: '请提供须解决时间', trigger: 'change'}],
          summary: [
            {required: true, message: '请输入线上问题摘要', trigger: 'blur'},
        ...
        currentProblemRules: {
          status: [{required: true, message: '请选择线上问题状态', trigger: 'change'}],
          priority: [{required: true, message: '请选择优先级', trigger: 'change'}],
          influence: [{required: true, message: '请选择严重程度', trigger: 'change'}],
          responser: [{required: true, message: '请选择负责人', trigger: 'change'}],
          typeFirst: [{required: true, message: '请选择一级分类', trigger: 'change'}],
      ...
      reqCreate(proId, callback) {
        ...
        .then(function (res) {
          if (res.data > 0) {
            ...
            .then(() => {
              _self.$router.push({name: "request", params: {id: res.data}});
            })
            .catch(() => {
              if (typeof callback == "function") {
                callback();
