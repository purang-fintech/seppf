  ...
  <div id="buildList">
    ...
    <el-card class="box-card" shadow="hover">
      <div slot="header" class="clearfix">
        ...
        <el-button v-no-more-click style="float:right; padding:3px 0;margin-left:12px" type="text" icon="el-icon-right" @click="releaseQuery(),dialogBuildVisible = true">立即构建
        </el-button>
      ...
      <div v-if="buildResults.length > 0">
        <el-collapse v-model="activeNames" v-for="buildResult  in buildResults" :key="buildResult.tag">
          <el-collapse-item :title="'构建时间: '+buildResult.buildHistories[0].createdDate" :name="buildResult.tag">
            <el-table :data="buildResult.buildHistories" empty-text="暂无构建记录">
      ...
      <el-form :inline="true" :model="buildEnv" :rules="rules" ref="buildEnv" label-width="130px">
        <el-form-item label="选择构建分支:" prop="prodBranch">
          <el-select v-model="buildEnv.prodBranch" clearable placeholder="请选择" @change="changeBranch" size="small">
            <el-option v-for="prodBranch in prodBranchs" :value="prodBranch.branchId" :key="prodBranch.branchId" :label="prodBranch.branchName"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="选择构建环境:" prop="envType">
          <el-select v-model="buildEnv.envType" placeholder="请选择" @change="changeEnv" size="small">
            <el-option v-for="item in envTypes" :value="item.typeId" :key="item.typeId" :label="item.typeName"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="Git分支:" prop="gitBranch">
          <el-select v-model="buildEnv.gitBranch" size="small" disabled>
            <el-option v-for="item in releases" :value="item.label" :key="item.value" :label="item.label"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <el-form
        :rules="rules"
        ref="buildEnvInstance"
            ...
            <el-tab-pane v-for="instance in buildEnv.instances" :key="instance" :label="instance">
              <el-tabs tab-position="top" type="card">
                <el-tab-pane v-for="buildFile in buildFiles[instance]" :label="buildFile.paramKey" :key="buildFile.id">
                  <el-switch active-color="#13ce66" active-text="增量" inactive-color="#3c7bdb" inactive-text="全量" v-model="buildMethod[instance]">
                  </el-switch>
                  <el-input
                    disabled
                    v-if="buildMethod[instance]"
                    v-model="buildFile.paramValue"
                    type="textarea"
                    :rows="8"
...
export default {
  ...
  methods: {
    ...
    buildRunSubmit: function () {
      let _self = this;
      let selectedEnv = _self.envTypes.find(d => {
        return d.typeId == _self.buildEnv.envType
      });
