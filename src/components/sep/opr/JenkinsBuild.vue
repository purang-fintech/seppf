  ...
  <div>
    <el-card class="box-card" shadow="hover">
      <div slot="header" class="clearfix">
        ...
        <el-button style="float:right; padding:3px 0;margin-left:12px" type="text"
                   @click="openBuildDeployDialog">构建部署
        </el-button>
        <el-button style="float:right; padding:3px 0;margin-left:12px" type="text"
                   @click="openOnlyBuildDialog">仅构建
        </el-button>
      ...
      <div v-if="buildHistories.length > 0">
        <el-table :data="buildHistories.slice((buildCurrentPage-1)*buildPagesize,buildCurrentPage*buildPagesize)"
          size="small" stripe  empty-text="暂无构建记录">
            ...
            <template slot-scope="scope">
              ...
              <div v-else-if="scope.row.buildStatus== 'FAILURE'">
                <router-link target="_blank"
                             :to="'/buildLog?jobName='+scope.row.jobName+'&buildVersion='+scope.row.buildVersion+'&buildStatus='+scope.row.buildStatus+'&instanceType='+instanceType">
    ...
    <el-card class="box-card" style="margin-top: 10px">
      ...
      <el-steps :active="pipelineStages.length" align-center>
        <el-step :title="stage.name" v-for="stage in pipelineStages"
                 :description="stage.status+' 耗时:'+stage.durationMillis/1000+'秒'" :key="stage.id"></el-step>
      </el-steps>
    </el-card>
    <el-card class="box-card" style="margin-top: 10px">
      <div slot="header" class="clearfix">
      ...
      <div>
        <el-table
          :data="deploymentHistories.slice((deployCurrentPage-1)*deployPagesize,deployCurrentPage*deployPagesize)"
          style="width: 100%">
          <el-table-column
            prop="id"
            label="部署版本"
          ...
          <el-table-column
            prop="userName"
            label="部署人" align="center">
          </el-table-column>
          <el-table-column
            prop="createdDate"
            label="部署时间" align="center">
          </el-table-column>
        </el-table>
      </div>
      <div class="paging" style="float: right;margin-top: 5px">
    ...
    <!--选择版本-->
    <el-dialog :close-on-click-modal="modalClose"
      title="选择部署版本"
      size="small"
      ...
      <el-table
        ...
        @current-change="handleDeployVersionChange"
        ref="deployTable"
    ...
    <el-dialog :close-on-click-modal="modalClose"
      ...
      <el-form :label-position="labelPosition" label-width="20%" :model="formLabelAlign"
               v-for="buildParam in buildParams" :key="buildParam.paramName">
        ...
        <el-form-item :label="buildParam.name" v-if="buildParam.type === 'ChoiceParameterDefinition'">
          ...
          <el-select v-model="buildParam.defaultParameterValue.value" v-else>
            <el-option v-for="choice in buildParam.choices" :label="choice" :value="choice"
                       :key="choice"></el-option>
          </el-select>
          <span>{{buildParam.description}}</span>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      startBuild() {
        ...
        let selectedEnv = env.find(d => {return d.typeId == _self.envType});
