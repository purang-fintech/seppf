<template>
  <div id="responserCharts" style="width: 99%; height: 300px;"></div>
</template>
...
export default {
  ...
  created() {
    let _self = this;
    let dresponsers = _self.sortData(_self.dresponsers, "responser", "children");
    let usersData = [];
    let legendData = [];
    let seriesData = [];

    dresponsers.forEach(data => {
  ...
  mounted() {
    let _self = this;
    let charts = document.getElementById("responserCharts");
